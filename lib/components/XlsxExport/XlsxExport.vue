<template>
  <CustomContentModal
    ...
    title="Aktiális oldal exportálása" 
    ...
    <template #content>
      <div class="flex flex-col mt-1">
        ...
        <div class="flex flex-row justify-end gap-2">
          <button @click="props.hideModal" class="disabled:opacity-60 btn min-w-[7rem] !rounded-full h-9 border border-slate-300 font-medium text-slate-800 hover:bg-slate-150 focus:bg-slate-150 active:bg-slate-150/80">
            {{ i18n.l('cancel') }}
          </button>
          <jsonExcel
            :data="sheetData"
...
//           var cSheetData = {};
//           for(var j in state.selected) {
//             if(state.selected[j].customExportFn) {
//               //console.log(state.selected[j])
